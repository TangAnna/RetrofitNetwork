...
import com.example.tang.androidframe.http.httpmodel.NoBodyModel;
...
public final class NobodyConverterFactory extends Converter.Factory {
    ...
    public Converter<ResponseBody, ?> responseBodyConverter(Type type, Annotation[] annotations,
                                                            Retrofit retrofit) {
        //判断当前的类型是否是我们需要处理的类型
        if (NoBodyModel.class.equals(type)) {
            //是则创建一个Converter返回转换数据
            return new Converter<ResponseBody, NoBodyModel>() {
                @Override
                public NoBodyModel convert(ResponseBody value) throws IOException {
